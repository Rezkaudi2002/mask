...
export async function POST(req: Request) {
  ...
  try {
    ...
    const systemEmailContent = `
      ...
      <p><strong>電話の許可:</strong> ${phonePermission === "allow_phone_call" ? "はい" : "いいえ"
      }</p>
      <p><strong>使用状況:</strong> ${usageType === "business" ? "事業（個人事業者または法人）" : "個人で使用"
      }</p>
      <p><strong>インボイス登録:</strong> ${invoiceRegistration === "registered" ? "はい" : "いいえ"
      }</p>
      <p><strong>登録番号の提供:</strong> ${provideRegistrationNumber === "will_provide" ? "はい" : "いいえ"
      }</p>
      <p><strong>都道府県:</strong> ${city}</p>
      <p><strong>商品情報:</strong> ${product_info}</p>
      <p><strong>査定希望商品の詳細:</strong> ${product_details}</p>
      <p><strong>商品の状態:</strong> ${product_condition === "scrap"
        ? "スクラップ"
    ...
    const userEmailContent = `
      ...
      <ul>
        ...
        <li><strong>商品の状態:</strong> ${product_condition === "scrap"
        ? "スクラップ"
        : product_condition === "used"
          ? "中古"
          : "新品"
